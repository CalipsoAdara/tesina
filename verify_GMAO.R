...
t = (acc[,,1] * sqrt(n_eff - 2)) / sqrt(1-acc[,,1]^2)

# Significancia de 0.05 y grados de libertad 
qt(p=0.05, df=c(12,22))
critc = abs(qt(p=0.05, df = trunc(n_eff)))
test = t < critc
d=ggDataFrame(acc[,,1])
GraphDiscretePuntos(d, ArLogic = test, Titulo = "WEEK 1 GMAO tasa", 
                    Breaks = seq(0,1,0.20), Label = "ACC",Paleta = "YlOrRd",Direccion = "1" )
