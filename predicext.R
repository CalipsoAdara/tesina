...
# Hago la resta
resta90 = pred90*100-prednoext*100
resta10 = pred10*100-prednoext*100

# Hago el promedio semanal para cada leadbin
predsem90= apply(resta90, c(1,2,4), mean, na.rm = T)
predsem10= apply(resta10, c(1,2,4), mean, na.rm = T)

predsem = list(predsem10,predsem90)
plabel = c("10","90")
for (pred in 1:length(predsem)) { # Por cada resta 
  p = predsem[[pred]]
  # convierto a data frame para las 4 weeks
  dimnames(p) <- list("x" = seq(265,330,1), "y" = rev(seq(-60,15,1)),"z" = c("1-7 Dias","8-14 Dias","15-21 Dias"))
  #dimnames(p) <- list("x" = seq(265,330,1), "y" = rev(seq(-60,15,1)),"z" = c("1-7 Days","8-14 Days","15-21 Days"))
  
  df <- reshape2::melt(p)
  colnames(df) <- c("x","y","week","z")
  
  g<-GraphDiscreteMultiple(Data=df,Breaks = seq(-25,25,5),Label = "p.p.",Paleta = "RdBu",Direccion = -1)
  g <- g + ggtitle(paste0("Predictibilidad SubX MJO Extremo - NoExtremos \nPercentil ",plabel[pred]," T2MA (99-14, Oct-Abr)"))
  #g <- g + ggtitle(paste0("Predictability SubX MJO Extremes - NotExtremes \nPercentile ",plabel[pred]," T2MA (99-14, Oct-Apr)"))
  
  ggsave(filename = paste0(FilePath,"resta_pred_",plabel[pred],".png"),plot=g,width = 8, height = 4)
}
# convierto a data frame para las 4 weeks
dimnames(predsem90) <- list("x" = seq(265,330,1), "y" = rev(seq(-60,15,1)),
                          "z" = c("1-7","8-14","15-21"))

df <- reshape2::melt(predsem90)
colnames(df) <- c("x","y","week","z")
...
# grafico 
tmap = ggDataFrame(predsem[,,3])
GraphDiscrete(tmap,Breaks = seq(0,1,0.20),Label = "p.p.",Paleta = "Greens",Direccion = 1, Titulo = "p")


GraphDiscrete(Data=df[df$week=="8-14",],Breaks = seq(-20,20,5),Label = "p.p.",Paleta = "Greens",Direccion = 1, Titulo = "p")
g<-GraphDiscreteMultiple(Data=df,Breaks = seq(-25,25,5),Label = "p.p.",Paleta = "RdBu",Direccion = -1)
