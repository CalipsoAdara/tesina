# SubX data - T2M anomaly verification for ECCC model (SubX database)
#
# M. Alvarez - 2020
...
#  Main Program  
#---------------------------------------------------------------------------------------
ar.model = readRDS("/home/lucia.castro/SubX_processed_Rdata/model_ECCC_ONDEFM.rds")
targetdate = readRDS("/home/lucia.castro/SubX_processed_Rdata/targetdate_ECCC_ONDEFM.rds")
...
for (week in 1:4) {
  for (lon in 1:66) {
    for (lat in 1:76) {
      ...
      desvio <- sd(observ,modelo)
      acc[lon,lat,week] <- coef_corr/desvio
...
fig <- grid.arrange(g1,g2,g3,g4, ncol = 1,top = textGrob("SubX ECCC-GEM tasa (99-15, Oct-Mar)",gp=gpar(fontsize=13,font=3)))
...
saveRDS(model_media_semanal, paste0("./SubX_processed_Rdata/modelweek_ECCC.rds"))
saveRDS(anom_media_semanal, paste0("./SubX_processed_Rdata/obsweek_ECCC.rds"))
saveRDS(metrics, "./SubX_processed_Rdata/metrics_ECCC.rds")